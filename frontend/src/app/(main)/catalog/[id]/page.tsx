...
function PlaceholderCover({ title, bookId }: { title: string; bookId: number }) {
  const palette = coverPalettes[bookId % coverPalettes.length];
  const initials = title.split(' ').slice(0, 2).map((w) => w[0]).join('').toUpperCase();
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-4 relative" style={{ background: palette.bg }}>
      <div className="absolute inset-6 border opacity-10" style={{ borderColor: palette.accent }} />
      <span className="font-serif text-6xl font-bold relative z-10" style={{ color: palette.accent }}>{initials}</span>
      <span className="text-[0.65rem] tracking-[0.2em] uppercase relative z-10 text-center px-6 leading-relaxed" style={{ color: palette.accent, opacity: 0.6 }}>{title}</span>
    </div>
  );
}
...
export default function BookDetailPage() {
  ...
  const handleFavoriteToggle = async () => {
    ...
    // Optimistic update
    setBook((prev) => prev ? { ...prev, is_favorited: !prev.is_favorited } : prev);
    setFavLoading(true);
    try {
  ...
  if (!book) {
    return (
      <div className="max-w-4xl mx-auto text-center py-32">
        <p className="font-serif text-2xl text-[#1a1714] mb-2">Book not found</p>
        <p className="text-sm text-[#8a7968] font-light mb-6">This book may have been removed.</p>
        <button
              ...
              {user && (
                ...
                >
                  {favLoading ? (
                    ...
                    <Heart size={13} className={book.is_favorited ? 'fill-[#d4b896]' : ''} />
                  )}
                  {book.is_favorited ? 'Saved to Favorites' : 'Add to Favorites'}
                </button>
              )}
